...
int main() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        if (is_sorted(v) == false) {
            std::cout << "Errore: Selection Sort ha fallito su un vettore di interi. \n";
            return EXIT_FAILURE;
        }
    }
    ...
    if (is_sorted(v_str) == false) {
        std::cout << "Errore: Selection Sort ha fallito sulle stringhe. \n";
        return EXIT_FAILURE;
    }

    std::cout << "Tutti i test per Selection Sort completati con successo. \n";
    return EXIT_SUCCESS;
}